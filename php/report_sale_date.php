<?php
    ...
    if($report_mode == "overview" && $selected_date != "none" && $selected_date != NULL){
	    $sql_where = " AND DATE(s.date_time) = '".$selected_date."' ";
	    $sql = " SELECT DATE(s.date_time) as selected_date, SUM(si.prod_amount) as amt, SUM((si.prod_price*si.prod_amount)-si.prod_discount) as sale_value 
		FROM sale_order_item si LEFT JOIN sale_order s ON si.order_id = s.order_id WHERE s.order_number NOT LIKE '%can%' 
