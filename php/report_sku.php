<?php
   ...
   if($report_mode == "alldate_daterange" && $date_from != "none" && $date_to != "none" && $date_from != NULL && $date_to != NULL){
	    ...
	    if(mysqli_num_rows($pre_result) > 0){    
		    ...
		    $selected_column = $selected_column.", t".$d_row[0].".amt".$d_row[0]." ";
		    $sql_repeat = $sql_repeat." LEFT JOIN (SELECT p.prod_id, SUM(si.prod_amount) as amt".$d_row[0]." 
		      FROM product p LEFT JOIN unit u ON p.unit_id = u.unit_id LEFT JOIN sale_order_item si ON p.prod_id = si.prod_id 
		      LEFT JOIN sale_order s ON si.order_id = s.order_id WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
		      AND DATE_FORMAT(s.date_time, '%Y%m%d') = '".$d_row[0]."' ".$sql_where." GROUP BY p.prod_id ORDER BY p.prod_id) as t".$d_row[0]." 
		      ON t.prod_id = t".$d_row[0].".prod_id";
       	    	}
    	    }
	    $sql = $sql1.$selected_column.$sql2.$sql_repeat." ORDER BY t.amt ".$order_seq;	    
    }
    if($report_mode == "alldate_selectedmonth"){
	    ...
	    $sql_where = " AND YEAR(s.date_time) = ".$year." AND MONTH(s.date_time) = ".$month." ";
    	    ...
    	    $sql2 = " FROM (SELECT p.prod_id, p.barcode, p.prod_name, si.prod_price, u.unit_name, SUM(si.prod_amount) as amt 
		      FROM product p LEFT JOIN unit u ON p.unit_id = u.unit_id LEFT JOIN sale_order_item si ON p.prod_id = si.prod_id 
		      LEFT JOIN sale_order s ON si.order_id = s.order_id WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
		      ".$sql_where." GROUP BY p.prod_id ORDER BY p.prod_id) as t ";
	    $sql_repeat = "";
    ...
    if($report_mode == "allmonth"){
	    ...
	    if(mysqli_num_rows($pre_result) > 0){    
			...
			for($i=0; $i<sizeof($b_list); $i++){
			    $selected_column = $selected_column.", t".$m_row[0]."_".$b_list[$i][0].".amt".$m_row[0]."_".$b_list[$i][0]." ";
			    $sql_repeat = $sql_repeat." LEFT JOIN (SELECT p.prod_id, SUM(si.prod_amount) as amt".$m_row[0]."_".$b_list[$i][0]." 
					...
					ON si.order_id = s.order_id WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
				        AND MONTH(s.date_time) = '".$m_row[0]."' AND s.branch_id = '".$b_list[$i][0]."' ".$sql_where." 
					GROUP BY p.prod_id ORDER BY p.prod_id) as t".$m_row[0]."_".$b_list[$i][0]." ON t.prod_id = t".$m_row[0]."_".$b_list[$i][0].".prod_id";
			}
		}
    	    }
	    $sql = $sql1.$selected_column.$sql2.$sql_repeat." ORDER BY t.amt ".$order_seq;
    ...
    if($report_mode == "allyear"){
	    ...
	    if(mysqli_num_rows($pre_result) > 0){    
		    ...
		    $sql_repeat = $sql_repeat." LEFT JOIN (SELECT p.prod_id, SUM(si.prod_amount) as amt".$y_row[0]." 
		      ...
		      for($i=0; $i<sizeof($b_list); $i++){
			    $selected_column = $selected_column.", t".$y_row[0]."_".$b_list[$i][0].".amt".$y_row[0]."_".$b_list[$i][0]." ";
			    $sql_repeat = $sql_repeat." LEFT JOIN (SELECT p.prod_id, SUM(si.prod_amount) as amt".$y_row[0]."_".$b_list[$i][0]." 
			      		FROM product p LEFT JOIN unit u ON p.unit_id = u.unit_id LEFT JOIN sale_order_item si ON p.prod_id = si.prod_id LEFT JOIN sale_order s 
					ON si.order_id = s.order_id WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
				        AND YEAR(s.date_time) = '".$y_row[0]."' AND s.branch_id = '".$b_list[$i][0]."' ".$sql_where." 
					GROUP BY p.prod_id ORDER BY p.prod_id) as t".$y_row[0]."_".$b_list[$i][0]." ON t.prod_id = t".$y_row[0]."_".$b_list[$i][0].".prod_id";
			}
