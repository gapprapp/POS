   ...
   if($report_mode == "alldate_daterange" && $date_from != "none" && $date_to != "none" && $date_from != NULL && $date_to != NULL){
	    ...
	    if(mysqli_num_rows($pre_result) > 0){    
		      ...
		      $sql = "SELECT c.customer_id, GROUP_CONCAT(DISTINCT s.payment_type, '|') as payment_history, 
			      ...
			      WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
		              AND c.customer_id = ".$cust_id." AND DATE_FORMAT(s.date_time, '%Y%m%d') = '".$d_row[0]."' GROUP BY c.customer_id ";	
		      $result = mysqli_query($conn, $sql);
	              if(mysqli_num_rows($result) > 0){
	       		  while($dd_row = mysqli_fetch_array($result)){
					$output[$d_count]["payment_history"] = $dd_row[1];
					$output[$d_count]["num_order"] = $dd_row[2];
					$output[$d_count]["num_item"] = $dd_row[3];
					$output[$d_count]["sale_value"] = $dd_row[4];
					...
					$output[$d_count]["payment_history"] = "-";
					$output[$d_count]["num_order"] = 0;
					$output[$d_count]["num_item"] = 0;
    ...
    if($report_mode == "allmonth"){
	    ...
	    for($m_count=1; $m_count<=12; $m_count++){
			...
			ON c.customer_id = s.customer_id LEFT JOIN sale_order_item si ON s.order_id = si.order_id 
			WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
		        AND c.customer_id = ".$cust_id." AND MONTH(s.date_time) = '".$m_count."' ".$sql_where." GROUP BY c.customer_id ";	
	    	$result = mysqli_query($conn, $sql);
	        if(mysqli_num_rows($result) > 0){
	       		while($m_row = mysqli_fetch_array($result)){
  				$output[$m_count]["payment_history"] = $m_row[1];
				$output[$m_count]["num_order"] = $m_row[2];
				$output[$m_count]["num_item"] = $m_row[3];
				$output[$m_count]["sale_value"] = $m_row[4];
			}
		}else{
			$output[$m_count]["payment_history"] = "-";
			$output[$m_count]["num_order"] = 0;
			$output[$m_count]["num_item"] = 0;
			$output[$m_count]["sale_value"] = 0;
		}
    	    }
    }
    if($report_mode == "allyear"){
	    ...
	    if(mysqli_num_rows($pre_result) > 0){    
       		while($y_row = mysqli_fetch_array($pre_result)){      
			    ...
			    ON c.customer_id = s.customer_id LEFT JOIN sale_order_item si ON s.order_id = si.order_id 
			    WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' 
		            AND c.customer_id = ".$cust_id." AND YEAR(s.date_time) = '".$y_row[0]."' ".$sql_where." GROUP BY c.customer_id ";	
	    	    $result = mysqli_query($conn, $sql);
	            if(mysqli_num_rows($result) > 0){
	       		    while($yy_row = mysqli_fetch_array($result)){
				$output[$y_count]["payment_history"] = $yy_row[1];
				$output[$y_count]["num_order"] = $yy_row[2];
				$output[$y_count]["num_item"] = $yy_row[3];
				$output[$y_count]["sale_value"] = $yy_row[4];
			        $output[$y_count]["year"] = $y_row[0]+543;
			    }
		    }else{
			$output[$y_count]["payment_history"] = "-";
			$output[$y_count]["num_order"] = 0;
			$output[$y_count]["num_item"] = 0;
