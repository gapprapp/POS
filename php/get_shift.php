<?php
    ...
    AND order_number NOT LIKE '%can%' AND date_time BETWEEN (SELECT date_time FROM cash_record 
    WHERE date_time = (SELECT max(date_time) FROM cash_record WHERE date_time < '$date') AND branch_id = '$b_id')
    AND '$date'";
    ...
    AND date_time BETWEEN (SELECT date_time FROM cash_record WHERE date_time = (SELECT max(date_time) FROM cash_record 
    WHERE date_time < '$date') AND branch_id = '$b_id') AND '$date'";
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result) > 0){
		while($row = mysqli_fetch_array($result)){
			$output[] = $row;
