<?php
    ...
    if($mode_matched==1){
	    if($report_case=="customer"){
		    $sql = "SELECT t.customer_id, t.customer_name, t.payment_history, SUM(t.num_order) as num_order, SUM(t.num_item) as num_item, 
			    SUM(t.sale_value) as sale_value 
			   FROM (SELECT c.customer_id, c.customer_name, GROUP_CONCAT(DISTINCT s.payment_type, '|') as payment_history, 
		     	   IFNULL(COUNT(DISTINCT si.order_id),0) as num_order, IFNULL(COUNT(DISTINCT si.prod_id),0) as num_item, 
 		           IFNULL(SUM(si.prod_price*si.prod_amount)-s.total_discount,0) as sale_value FROM customer c LEFT JOIN sale_order s 
		           ...
		           GROUP BY c.customer_id, s.order_id ORDER BY s.order_id) as t 
			   GROUP BY t.customer_id ORDER BY num_order ".$order_seq." LIMIT ".$start.",100";
	    }
	    if($report_case=="branch"){
		    $sql = "SELECT t.branch_id, t.branch_name, t.payment_histoty, SUM(t.num_order) as num_order, SUM(t.num_item) as num_item,
			    SUM(t.sale_value) as sale_value 
			   FROM (SELECT b.branch_id, b.branch_name, GROUP_CONCAT(DISTINCT s.payment_type, '|') as payment_history, 
		     	   IFNULL(COUNT(DISTINCT si.order_id),0) as num_order, IFNULL(COUNT(DISTINCT si.prod_id),0) as num_item, 
 		           IFNULL(SUM(si.prod_price*si.prod_amount)-s.total_discount,0) as sale_value FROM branch b LEFT JOIN sale_order s 
		           ON b.branch_id = s.branch_id LEFT JOIN sale_order_item si ON s.order_id = si.order_id 
		           WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' ".$sql_where." 
		           GROUP BY b.branch_id, s.order_id ORDER BY s.order_id) as t
			   GROUP BY t.branch_id ORDER BY t.num_order ".$order_seq." LIMIT ".$start.",100";
	    }		
	    if($report_case=="payment_type"){
		    $sql = "SELECT t.payment_type, SUM(t.num_order) as num_order, SUM(t.num_item) as num_item,
			    SUM(t.sale_value) as sale_value 
			   FROM (SELECT DISTINCT s.payment_type, IFNULL(COUNT(DISTINCT si.order_id),0) as num_order, 
			   IFNULL(COUNT(DISTINCT si.prod_id),0) as num_item, IFNULL(SUM(si.prod_price*si.prod_amount)-s.total_discount,0) as sale_value 
			   FROM sale_order s LEFT JOIN sale_order_item si ON s.order_id = si.order_id 
		           WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' ".$sql_where." 
			   GROUP BY s.payment_type, s.order_id ORDER BY s.order_id) as t 
		           GROUP BY t.payment_type ORDER BY t.num_order ".$order_seq." LIMIT ".$start.",100";
	    }
	    if($report_case=="user"){
		    $sql = "SELECT t.user_id, t.user_name, t.payment_histoty, SUM(t.num_order) as num_order, SUM(t.num_item) as num_item,
			    SUM(t.sale_value) as sale_value 
			   FROM (SELECT u.user_id, u.user_name, GROUP_CONCAT(DISTINCT s.payment_type, '|') as payment_history, 
		     	   IFNULL(COUNT(DISTINCT si.order_id),0) as num_order, IFNULL(COUNT(DISTINCT si.prod_id),0) as num_item, 
 		           IFNULL(SUM(si.prod_price*si.prod_amount)-s.total_discount,0) as sale_value FROM user u LEFT JOIN sale_order s 
		           ON u.user_id = s.user_id LEFT JOIN sale_order_item si ON s.order_id = si.order_id 
		           WHERE s.order_number NOT LIKE '%can%' AND si.order_id NOT LIKE '%refund%' ".$sql_where." 
			   GROUP BY u.user_id, s.order_id ORDER BY s.order_id) as t 
		           GROUP BY t.user_id ORDER BY t.num_order ".$order_seq." LIMIT ".$start.",100";
	    }
    }
